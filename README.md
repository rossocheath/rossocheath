<div align="center">

# Ros Socheath

### Laravel Developer • SaaS Builder • System Administrator

Building modern web platforms, automation tools, and production-ready Laravel applications.

![Profile Views](https://komarev.com/ghpvc/?username=rossocheath&style=flat-square)
![GitHub followers](https://img.shields.io/github/followers/rossocheath?style=flat-square)
![GitHub stars](https://img.shields.io/github/stars/rossocheath?style=flat-square)

</div>

---

## About Me

I'm a developer from Cambodia focused on building practical, clean, and scalable web applications.

I work mainly with **Laravel**, **Livewire**, **Tailwind CSS**, **Flux UI**, **FilamentPHP**, and modern deployment stacks.  
My current focus is building SaaS products, admin systems, static site hosting tools, and content platforms.

---

## Tech Stack

**Backend**  
`Laravel` · `PHP` · `Livewire` · `FilamentPHP` · `REST API` · `MySQL` · `PostgreSQL` · `Redis`

**Frontend**  
`Tailwind CSS` · `Flux UI` · `Alpine.js` · `Blade` · `JavaScript` · `Monaco Editor`

**DevOps and Tools**  
`Ubuntu Server` · `Nginx` · `Docker` · `Supervisor` · `Cloudflare` · `Git` · `GitHub`

---

## Current Projects

### The SiteHub

A modern static site builder and hosting platform built with Laravel, Livewire, Flux UI, and Tailwind CSS.

### The AniView

An anime and manga discovery platform with multilingual UI, SEO support, AI article automation, and curated legal media links.

### University and MIS Systems

Building and improving internal systems, exam management tools, admin dashboards, and production deployment workflows.

---

## What I Care About

- Clean and maintainable code
- Professional UI and UX
- Secure production deployment
- SEO-ready web platforms
- Practical tools that solve real problems
- Building products that can grow into real businesses

---

## Featured Skills

```php
$developer = [
    'name' => 'Ros Socheath',
    'role' => 'Laravel Developer',
    'focus' => ['SaaS', 'Admin Systems', 'Automation', 'Production Deployment'],
    'country' => 'Cambodia',
    'stack' => ['Laravel', 'Livewire', 'Tailwind CSS', 'Flux UI', 'FilamentPHP'],
];
```

---

## Connect With Me

<div align="left">

[![GitHub](https://img.shields.io/badge/GitHub-rossocheath-181717?style=for-the-badge&logo=github)](https://github.com/rossocheath)
[![Email](https://img.shields.io/badge/Email-Contact%20Me-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:rossocheat1@gmail.com)

</div>
